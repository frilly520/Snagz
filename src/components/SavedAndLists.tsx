...
export const SavedAndLists: React.FC<SavedAndListsProps> = ({
  ...
  return (
    <div id="saved-and-lists" className="space-y-6">
      {/* Tab Selector */}
      <div className="flex items-center gap-1.5 p-1 rounded-xl bg-[#141926] border border-[#222b3e] max-w-2xl overflow-x-auto">
        <button
          type="button"
      ...
      {activeTab === 'saved' && (
        <div>
          {savedDeals.length === 0 ? (
            <div className="p-12 text-center rounded-xl bg-[#121624] border border-[#222b3e]">
              <Heart className="w-10 h-10 text-slate-500 mx-auto mb-3" />
              <h3 className="font-bold text-white text-base">No saved deals yet</h3>
              <p className="text-xs text-slate-400 mt-1">Tap the heart icon on any coupon or deal to keep track of it here.</p>
            </div>
          ) : (
      ...
      {activeTab === 'lists' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-white">Your Curated Shopping Lists</h2>
            <button
              type="button"
              onClick={() => setShowCreateListModal(true)}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-sm transition-colors"
            >
          ...
          <div className="space-y-4">
            {userLists.map((list) => {
              ...
              return (
                <div key={list.id} className="p-5 rounded-xl bg-[#121624] border border-[#222b3e]">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-[#1f2638]">
                    <div>
                      <div className="flex items-center gap-2">
                        <Bookmark className="w-4 h-4 text-blue-400" />
                        <h3 className="font-bold text-white text-base">{list.name}</h3>
                        <span className="text-xs text-slate-400 font-mono">({listDeals.length} items)</span>
                      </div>
                      {list.description && (
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#141926] hover:bg-[#1a2133] border border-[#222b3e] text-xs text-slate-300 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-[#141926] hover:bg-rose-950/60 text-slate-400 hover:text-rose-400 border border-[#222b3e] transition-colors"
                      >
                      ...
                      {listDeals.map((deal) => (
                        <div key={deal.id} className="relative">
                          ...
                          <button
                            ...
                            className="absolute top-2 right-12 z-10 p-1.5 rounded-lg bg-[#0d101a]/80 hover:bg-rose-950 text-slate-400 hover:text-rose-400 border border-[#222b3e]"
                          >
      ...
      {activeTab === 'watchlist' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-white">Price Drop Product Watchlist</h2>
            <button
              type="button"
          ...
          <div className="space-y-3">
            {watchlist.map((item) => (
              <div key={item.id} className="p-4 rounded-xl bg-[#121624] border border-[#222b3e] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <h4 className="font-bold text-white text-sm">{item.productName}</h4>
                  <div className="flex items-center gap-3 mt-1 text-xs text-slate-400">
                    <span>Store: <strong className="text-slate-200">{item.bestStore}</strong></span>
                    <span>• Target Price: <strong className="text-blue-400 font-mono">${item.targetPrice.toFixed(2)}</strong></span>
                    <span>• Current Best: <strong className="text-white font-mono">${item.currentBestPrice.toFixed(2)}</strong></span>
                  </div>
                </div>

                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-xs px-2.5 py-1 rounded-lg bg-blue-950/60 border border-blue-500/30 text-blue-300 font-medium">
                    {item.activeCouponsCount} Active Promo Codes
                  </span>
                  <button
                    type="button"
                    onClick={() => onRemoveFromWatchlist(item.id)}
                    className="p-2 rounded-lg bg-[#141926] hover:bg-rose-950/60 text-slate-400 hover:text-rose-400 border border-[#222b3e]"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
      ...
      {activeTab === 'alerts' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-white">Automated Savings Alerts</h2>
            <button
              type="button"
          ...
          <div className="space-y-3">
            {alerts.map((alert) => (
              <div key={alert.id} className="p-4 rounded-xl bg-[#121624] border border-[#222b3e] flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-400">
                      ...
                      <span className="text-blue-400 font-semibold">{alert.matchCount} current matches</span>
                    </div>
                  </div>
                </div>
      ...
      {showCreateListModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#121624] border border-[#222b3e] rounded-xl p-5 sm:p-6 shadow-2xl">
            <h3 className="text-base font-bold text-white mb-4">Create New Shopping List</h3>
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-[#0d101a] border border-[#222b3e] text-white text-xs focus:border-blue-500 focus:outline-none"
                />
              </div>
              <div>
              ...
              <div className="flex items-center justify-end gap-2 pt-3">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg bg-[#141926] text-slate-300 text-xs hover:text-white border border-[#222b3e]"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-1.5 rounded-lg bg-blue-600 text-white font-bold text-xs hover:bg-blue-500 transition-colors disabled:opacity-50"
                >
                  Create List
                </button>
              </div>
      ...
      {showWatchlistModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#121624] border border-[#222b3e] rounded-xl p-5 sm:p-6 shadow-2xl">
            <h3 className="text-base font-bold text-white mb-4">Track Price Drop on Product</h3>
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-[#0d101a] border border-[#222b3e] text-white focus:border-blue-500 focus:outline-none"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="block font-semibold text-slate-300 mb-1">Current Price ($)</label>
                  <input
                    type="number"
      ...
      {showAlertModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#121624] border border-[#222b3e] rounded-xl p-5 sm:p-6 shadow-2xl">
            <h3 className="text-base font-bold text-white mb-4">Create Automated Deal Alert</h3>
            <div className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-300 mb-1">Keyword or Brand</label>
                <input
                  type="text"
                  value={alertQuery}
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="block font-semibold text-slate-300 mb-1">Min Discount (%)</label>
                  <input
                    type="number"
