...
export const ShoppingTripView: React.FC<ShoppingTripViewProps> = ({ onBackToDeals }) => {
  ...
  return (
    <div id="shopping-trip-view" className="w-full max-w-6xl mx-auto px-4 py-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-5 border-b border-[#1f2638]">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400">Smart Trip Routing</span>
          </div>
        ...
        <div className="flex items-center gap-2">
          {plan && (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-[#141926] border border-[#222b3e] hover:bg-[#1a2133] text-slate-200 text-xs font-semibold flex items-center gap-1.5 transition-colors"
            >
              {copiedPlan ? <Check className="w-3.5 h-3.5 text-blue-400" /> : <Copy className="w-3.5 h-3.5" />}
          ...
          {onBackToDeals && (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-[#141926] hover:bg-[#1a2133] border border-[#222b3e] text-slate-200 text-xs font-semibold transition-colors"
            >
              Back to Deals
            </button>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="p-5 rounded-xl bg-[#121624] border border-[#222b3e]">
            <h2 className="text-sm font-bold text-white mb-3 flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setItems([])}
                className="text-xs text-slate-500 hover:text-rose-400 font-normal transition-colors"
              >
                Clear all
              </button>
            </h2>
            ...
            >
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg bg-[#0d101a] border border-[#222b3e] text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500"
              />
              <button
                id="btn-submit-add-item"
                type="submit"
                className="p-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shrink-0 transition-colors"
              >
            ...
            <div className="mb-4">
              <span className="text-[10px] uppercase font-mono text-slate-500 block mb-1.5">Quick Add:</span>
              <div className="flex flex-wrap gap-1.5">
                {quickSuggestions.map((sug) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-md bg-[#141926] hover:bg-[#1a2133] border border-[#222b3e] text-[11px] text-slate-300 transition-colors"
                  >
                    + {sug}
                  </button>
                ))}
            ...
            <div className="space-y-1.5 max-h-60 overflow-y-auto pr-1">
              {items.map((item, idx) => (
                <div 
                  key={idx}
                  className="flex items-center justify-between px-3 py-2 rounded-lg bg-[#0d101a] border border-[#222b3e] text-xs text-slate-200"
                >
          ...
          {/* Mode Switcher */}
          <div className="p-4 rounded-xl bg-[#121624] border border-[#222b3e]">
            <h3 className="text-xs font-bold text-slate-300 mb-2">Trip Preference</h3>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setOptimizationMode('MAXIMUM_SAVINGS')}
                className={`p-2.5 rounded-lg border text-xs font-semibold flex flex-col items-center gap-1 transition-colors ${
              ...
              >
                ...
                <span className="text-[10px] font-normal text-slate-500">Split to save most</span>
              </button>
              ...
              >
                ...
                <span className="text-[10px] font-normal text-slate-500">Single convenient stop</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          {loading ? (
            <div className="p-12 text-center bg-[#121624] border border-[#222b3e] rounded-xl">
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 sm:p-5 rounded-xl bg-[#121624] border border-[#222b3e]">
                ...
                <div className="grid grid-cols-2 gap-3 text-center">
                  <div className="p-3 bg-[#0d101a] rounded-lg border border-[#222b3e]">
                    <span className="text-[11px] text-slate-400 block">Single-Store Total</span>
                  ...
                  <div className="p-3 bg-blue-950/40 rounded-lg border border-blue-500/30">
                    <span className="text-[11px] text-blue-300 block">Optimal Split Total</span>
                    <span className="text-lg font-bold text-blue-400 font-mono">
                      ${plan.multiStoreOption.totalPrice.toFixed(2)}
                    </span>
                    <span className="text-[10px] text-blue-300 block mt-0.5 font-semibold">
              ...
              <div className="p-5 rounded-xl bg-[#121624] border border-blue-500/50">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-3">
                  {plan.multiStoreOption.stores.map((store, sIdx) => (
                    <div key={sIdx} className="p-3.5 rounded-lg bg-[#0d101a] border border-[#222b3e] text-xs">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-500 text-[11px]">({store.itemCount} items)</span>
                        </div>
                        <span className="font-mono font-bold text-blue-400">${store.totalPrice.toFixed(2)}</span>
                      </div>

                      <div className="space-y-1.5 pl-7 border-l border-[#222b3e] my-2">
              ...
              <div className="p-4 rounded-xl bg-[#121624] border border-[#222b3e] text-xs">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-bold text-slate-300">Single-Store Direct Trip</h4>
