...
import { translateSearchIntent, askDealAssistant, analyzeReceipt } from './server/ai';
import { runIngestionPipeline } from './server/pipeline';
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', time: new Date().toISOString(), totalDeals: db.deals.length });
});
...
app.post('/api/extension/match', (req, res) => {
  const { url = '', storeDomain = '', cartTotal = 0 } = req.body;
...
// Explicit 404 handler for unmatched /api requests to prevent falling through to Vite HTML
app.all('/api/*', (req, res) => {
  res.status(404).json({ error: `API route not found: ${req.method} ${req.path}` });
});

// Global error handler ensuring all API errors return JSON
